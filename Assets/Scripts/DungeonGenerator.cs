...
public class DungeonGenerator : MonoBehaviour
{
    ...
    [SerializeField]
    private ClearAndBakeNavMesh clearAndBakeNavMesh;

    //private bool isGenerated = false;
    
    private void Awake()
    {
        ClearAndBakeNavMesh clearAndBakeNavMesh = GetComponent<ClearAndBakeNavMesh>();
    }
    void Start()
    {
    ...
    {
        StartCoroutine(GenerateAndBake());
        //clearAndBakeNavMesh.CreateBake();
    }

    private IEnumerator GenerateAndBake()
    {
        clearAndBakeNavMesh.ClearNavMesh();

        Generate();
        GenerateAlternativeEntrances();
        FillEmptyEntrances();

        yield return null; // Espera 1 frame para que Unity actualice todo

        Debug.Log("BAKE DESPUÉS DE ESPERAR UN FRAME");
    ...
    //
    //
    //    Debug.Log("vamos al createbake?");
    //
    //}

    private void Generate()
    {
        //if (noOfRooms != generatedRooms.Count) {
        for (int i = 0; i < noOfRooms - alternateEntrances.Count; i++) //un for para crear el numero de cuartos deseados
        {
            ...
            {
                int randomIndex = Random.Range(0, entrance.Count);
            ...
            else
            {
                ...
                bool shouldPlaceHallway = Random.Range(0f, 1f) > 0.7f;
                ...
                {
                    if (!MapHasAnyAvailableEntrypoint())
                    {
                        Debug.Log("Mapa sellado. Terminamos generación.");
                        break;
                    }
                }
    ...
    private bool RetryPlacement(GameObject itemToPlace, GameObject doorToPlace)
    {
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++)
        {
            ...
            while (randomGeneratedRoom == null && retries < 20)
            {
        ...
        Debug.Log("Falló después de 10 intentos → destruimos pieza");
    ...
    private bool MapHasAnyAvailableEntrypoint()
    {
        foreach (DungeonPart part in generatedRooms)
        {
            if (part.HasAnyAvailableEntrypoint())
                return true;
        }
